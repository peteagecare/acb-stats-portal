...
/** Fire an in-app notification + email when a workspace task gets completed,
 *  letting the task creator know — but only if the actor is someone other
 *  than the creator. */
export async function notifyTaskCompleted(opts: {
  ...
  const { taskId, taskTitle, projectId, recipientEmail, actorEmail, origin } = opts;
  if (!recipientEmail || recipientEmail.toLowerCase() === actorEmail.toLowerCase()) return;
  ...
  const wantInApp = pref?.taskCompletedInApp ?? true;
  const wantEmail = pref?.taskCompletedEmail ?? true;
